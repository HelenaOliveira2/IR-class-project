...
def get_admin_dashboard_stats():
    ...
    try:
        ...
        return {
            ...
            "frequentTerms": top_terms, # 🔥 AGORA SÃO AS PALAVRAS REAIS DOS TEUS PDFS
            "indexGrowth": [
                { "month": "Jan", "size": 0.5 },
                { "month": "Fev", "size": 1.2 },
                { "month": "Mar", "size": 2.8 },
                { "month": "Abr", "size": 4.5 },
                { "month": "Mai", "size": file_size_mb } # 🔥 O ÚLTIMO MÊS TEM O TAMANHO REAL DO JSON!
            ],
            "classification": real_metrics # 🔥 PERCENTAGENS CALCULADAS NA HORA!
        }
    except Exception as e:
...
def calculate_real_ir_metrics(engine_instance):
    """
    Calcula Precision, Recall, F1-Score e Accuracy reais executando 
    queries de teste contra um Ground Truth conhecido.
    """
    # 🌟 O TEU GROUND TRUTH: 
    # Altera os IDs abaixo para IDs de documentos que tu SABES que existem 
    # na tua base de dados e que respondem perfeitamente a estas pesquisas.
    GROUND_TRUTH = {
        "neural networks": [1, 11, 31], # Ex: Docs que falam mesmo de Redes Neurais
        "health data": [7, 8, 36],      # Ex: Docs de saúde
        "textile industry": [9, 29]     # Ex: Docs da indústria têxtil
    }
    ...
    for query, expected_docs in GROUND_TRUTH.items():
        ...
        # 1. Fazemos a pesquisa real no teu motor (Pedimos o Top 10)
        # Assumindo que ranked_search devolve tuplos (id, score)
        ...
        # 2. As Matemáticas da Recuperação de Informação
        ...
        # 3. Precisão (Quantos dos que devolvemos eram relevantes?)
        precision = true_positives / (true_positives + false_positives) if (true_positives + false_positives) > 0 else 0
        
        # 4. Recall (Quantos dos relevantes nós conseguimos encontrar?)
    ...
    # 5. F1-Score (Média Harmónica)
    f1_score = 2 * (avg_precision * avg_recall) / (avg_precision + avg_recall) if (avg_precision + avg_recall) > 0 else 0

    # 6. Accuracy (Em IR, os Verdadeiros Negativos são todos os docs do corpus que não devolvemos)
    # Uma aproximação académica comum para a métrica geral de Accuracy:
